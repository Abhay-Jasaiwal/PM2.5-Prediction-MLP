...
# ================== USER INPUT ==================
FILE_PATH = "ITO_CPCB.csv"   # Change station here
STATION_NAME = "ITO"
...
def event_bar(event_col, title):
    ...
    plt.figure()
    plt.bar(x - width/2, [y[0] for y in yearly], width, label="Event (orange)")
    plt.bar(x + width/2, [y[1] for y in yearly], width, label="Non-Event (blue)")
...
for file in files:
    ...
    temp = temp.rename(columns={"PM2.5":"pm25"})
    means.append(temp["pm25"].mean())

# Sort for better readability
sorted_data = sorted(zip(station_names, means), key=lambda x: x[1], reverse=True)
station_names_sorted, means_sorted = zip(*sorted_data)

plt.figure(figsize=(14,6))
plt.bar(station_names_sorted, means_sorted)

plt.xlabel("Monitoring Stations")
plt.ylabel("Average PM2.5 (µg/m³)")
plt.title("Average PM2.5 Comparison Across 10 Monitoring Stations")
